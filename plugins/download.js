...
cmd({
  ...
  try {
    ...
    const caption = `╭━━〔 *TWITTER DOWNLODER* 〕
┃✦╭──────────────
...
cmd({
  ...
  try {
    if (!q) {
      return reply("*‼️Please provide a valid mediafire link.*");
    }
    ...
    if (!data || !data.status || !data.result || !data.result.dl_link) {
      return reply("*🏷️Please provide an mediafire link.Ensure the link is valid and public.*");
    }
    ...
    const caption = `╭━━〔 *MEDIAFIRE DOWNLODER* 〕
┃✦╭──────────────
┃✦│ *File name:* ${file_name}
┃✦│ *File type:* ${mime_type}
┃✦╰──────────────
...
cmd({
  ...
  try {
    if (!q) {
      return reply("*🏷️Please provide an app name to search.*");
    }
...
cmd({
  ...
  try {
    if (!q) {
      return reply("*🏷️Please provide a valid google drive link.*");
    }
    ...
    if (downloadUrl) {
      ...
      await conn.sendMessage(from, {
        ...
        caption: "> *© ᴩᴏᴡᴇʀᴇᴅ ʙʏ ᴛᴇᴄʜ-ʜᴏʀɪᴢᴏɴ*"
      }, { quoted: m });
